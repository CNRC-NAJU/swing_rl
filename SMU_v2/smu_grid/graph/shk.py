...
class SHK:
    ...
    def _bridge(self, new_node: int) -> None:
        ...
        while True:
            ...
            node1, node2 = self.rng.choice(list(self.graph.edges))
...
def get_shk(
    ...
    grid = SHK(init_num_nodes, num_nodes, SHKParams(p, q, r, s), rng)
    graph = grid.graph

    if include_pos:
        nx.function.set_node_attributes(graph, {node: pos for node, pos in enumerate(grid.pos)}, name="pos")
