...
template <typename T>
std::vector<T> flatten(const std::vector<std::vector<T>>& t_mat) {
    std::vector<T> flattened;
    for (const auto& vec : t_mat) {
...
std::vector<std::vector<T>> transpose(const std::vector<std::vector<T>>& t_mat) {
    ...
    for (uint64_t i = 0; i < t_mat.size(); ++i) {
        for (uint64_t j = 0; j < t_mat[0].size(); ++j) {
...
std::vector<T> operator+(const std::vector<T>& t_vec1, const std::vector<T>& t_vec2) {
    if (t_vec1.size() != t_vec2.size()) {
        std::cout << "In plus operation, two vectors have different length"
                  << std::endl;
        exit(1);
    }
...
std::vector<std::vector<T>> operator+(
    const std::vector<std::vector<T>>& t_mat1, const std::vector<std::vector<T>>& t_mat2
) {
...
template <typename T>
std::vector<std::vector<T>>& operator+=(
    std::vector<std::vector<T>>& t_mat1, const std::vector<std::vector<T>>& t_mat2
) {
    if (t_mat1.size() != t_mat2.size() || t_mat1[0].size() != t_mat2[0].size()) {
        std::cout << "In plus operation, two matrices have different size" << std::endl;
        exit(1);
...
template <typename T, typename TT>
std::vector<std::vector<T>> operator*(
    const TT& c, const std::vector<std::vector<T>>& t_mat
) {
...
template <typename T, typename TT>
std::vector<std::vector<T>> operator*(
    const std::vector<std::vector<T>>& t_mat, const TT& c
) {
    return c * t_mat;
}
template <typename T, typename TT>
std::vector<std::vector<T>>& operator*=(
    std::vector<std::vector<T>>& t_mat, const TT& c
) {
...
std::vector<std::vector<T>> product(
    const std::vector<T>& t_vec1, const std::vector<T>& t_vec2
) {
...
std::vector<T> product(
    const std::vector<std::vector<T>>& t_mat, const std::vector<T>& t_vec
) {
    if (t_mat[0].size() != t_vec.size()) {
        std::cout << "In product between matrix and vector, size is different"
                  << std::endl;
        exit(1);
    }
    const unsigned n = t_mat.size();
...
std::vector<std::vector<T>> product(
    ...
    if (t_mat1[0].size() != t_mat2.size()) {
        std::cout << "In product between matrix and matrix, size is different"
                  << std::endl;
        exit(1);
    }
...
template <typename T>
std::vector<T> element_pow(const std::vector<T>& t_vec1, const std::vector<T>& t_vec2) {
    if (t_vec1.size() != t_vec2.size()) {
        std::cout << "In inner product, two vectors have different size" << std::endl;
        exit(1);
    }
...
template <typename T>
std::vector<std::vector<T>> element_pow(
    const std::vector<std::vector<T>>& t_mat1, const std::vector<std::vector<T>>& t_mat2
) {
    if (t_mat1.size() != t_mat2.size() || t_mat1[0].size() != t_mat2[0].size()) {
        std::cout << "In plus operation, two matrices have different size" << std::endl;
...
template <typename T>
std::vector<T> element_tanh(const std::vector<T>& t_vec) {
    std::vector<T> result;
    result.reserve(t_vec.size());
    for (const T& e : t_vec) {
...
template <typename T, typename TT>
std::vector<T> element_pow(const std::vector<T>& t_vec, const TT& c) {
    std::vector<T> result;
    result.reserve(t_vec.size());
    for (const T& e : t_vec) {
...
template <typename T, typename TT>
std::vector<T> element_pow(const TT& c, const std::vector<T>& t_vec) {
    std::vector<T> result;
    result.reserve(t_vec.size());
    for (const T& e : t_vec) {
...
std::vector<double> linspace(
    const double& t_begin, const double& t_end, const unsigned& t_size
) {
...
//* Pop back of vector
template <typename T>
void pop_back(std::vector<T>& t_vec) {
    t_vec.pop_back();
    return;
}
...
template <typename T, typename TT>
void sample_num(std::map<T, int>& t_map1, const std::map<T, TT>& t_map2) {
    for (const auto& e : t_map2) {
        ++t_map1[e.first];
    }
}
...
//* pop back of map
template <typename T, typename TT>
void pop_back(std::map<T, TT>& t_map) {
...
template <typename T>
void print(
    const std::vector<T>& t_vec,
    const int& t_precision = 6,
    const char& t_seperate = ','
) {
    t_precision < 0 ? std::cout.precision(std::numeric_limits<double>::digits10 + 1)
                    : std::cout.precision(t_precision);
    std::cout << "[";
...
template <typename T>
void print(
    const std::vector<std::vector<T>>& t_mat,
    const int& t_precision = 6,
    const char& t_seperate = ','
) {
    t_precision < 0 ? std::cout.precision(std::numeric_limits<double>::digits10 + 1)
