...
def get_step_solver(
    solver_name: str,
    weighted_adjacency_matrix: npt.NDArray[T] | coo_matrix,
    params: npt.NDArray[T],
    dt: npt.NDArray[T],
) -> Callable[[npt.NDArray, npt.NDArray], tuple[npt.NDArray, npt.NDArray]]:
    # Which module will be used to simulate
    if "sparse" in solver_name:
        solver_module = sparse
        if isinstance(weighted_adjacency_matrix, np.ndarray):
