...
class PPOAgent:
    ...
    def act(self, features, adj, weight, activity):
        amplitude, actions, dist, entropy = self._get_action(
            features, adj, weight, activity
        )
    ...
    def update(self, returns, advantages, ratios, ppo_eps, entropies, ent_coeff):
        ...
        actor_loss = -torch.sum((torch.minimum(l1, l2) + ent_coeff * entropies))
    ...
    def push(self, returns, advantages, ratios, ppo_eps, entropies, ent_coeff):
        ...
        self.actor_losses.append(
            (-(torch.minimum(l1, l2) + ent_coeff * entropies).sum())
        )
        # self.actor_losses.append((- (torch.minimum(l1, l2) + ent_coeff * entropies).mean()))

    def update_mean(self):
        actor_loss = torch.stack(self.actor_losses).mean()
    ...
    def gen_traj_train(
        ...
        while 1:
            ...
            if len(power[act_gen]) == 0:
                rewards[-1] = -1  # not for sure of this
                n_fail = node  # not for sure of this
                break
            if t > 2:
                break
            ...
            if df:
                ...
                amplitude, ratio, entropy = self.act(
                    feature, edge_list, edge_weight, act_gen
                )
                rebal_pm(
                    power,
                    mass,
                    amplitude.clone().detach().cpu().numpy(),
                    act_gen,
                    failed,
                )
                mass[failed] = m0
    ...
    def gen_traj_test(
        ...
        with torch.no_grad():
            ...
            rebal_pm(
                power, mass, amplitude.clone().detach().cpu().numpy(), act_gen, failed
            )
            mass[seed] = m0
            ...
            while 1:
                ...
                for n in range(len(power)):
                    if (dphase[n] > threshold or dphase[n] < -threshold) and activity[
                        n
                    ]:
                        failed[n] = True
                        activity[n] = False
