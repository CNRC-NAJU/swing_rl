...
from utils.swing_solver import *
...
class PPOAgent():
    ...
    def gen_traj_train(self, seed, edge_list, edge_weight, adjm, graph_torch, 
                 init_mask_gen, steady_phase, steady_dphase, init_power, init_activity, init_failed,
                 gamma, h, K, mass, threshold, node, max_step, ep_len, acceleration_torch):
        rewards = []
        ratios = []
        entropies = []

        mask_gen = np.copy(init_mask_gen)
        phase = steady_phase.clone().detach()
        dphase = steady_dphase.clone().detach()
        power = init_power.clone().detach()
        ...
        rebal(power, amplitude.clone().detach(), act_gen, failed)
        rewards.append(0)
        ratios.append(ratio)
        ...
        while 1:
            ...
            df = 0
            delta_dphase, delta_phase = rk4_torch(acceleration_torch, phase, dphase, power, h)
            phase += h * delta_phase
            dphase += h * delta_dphase
    ...
    def gen_traj_test(self, seed, edge_list, edge_weight, adjm, graph_torch, 
                 init_mask_gen, steady_phase, steady_dphase, init_power, init_activity, init_failed,
                 gamma, h, K, mass, threshold, node, max_step, ep_len, acceleration_torch):
