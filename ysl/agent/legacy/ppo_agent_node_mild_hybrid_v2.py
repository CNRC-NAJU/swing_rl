...
class PPOAgent():
    ...
    def gen_traj_train(self, seed, edge_list, edge_weight, adjm, 
                 init_mask_gen, steady_phase, steady_dphase, init_power, init_activity, init_failed,
                 gamma, h, K, init_mass, threshold, node, max_step, ep_len, init_hp):
        rewards = []
        ratios = []
        entropies = []
        ...
        failed[seed] = True
        feature = torch.tensor([[hp[n], gamma[n], mass[n], power[n], phase[n], dphase[n], int(failed[n]), hp[n], gamma[n], mass[n], power[n], phase[n], dphase[n], int(failed[n])] for n in range(node)]).float().to(self.device)
        act_gen = activity*mask_gen
        amplitude, ratio, entropy = self.act(feature, edge_list, edge_weight, act_gen)
        rebal_hp(power, amplitude.clone().detach().cpu().numpy(), act_gen, failed, hp, mass)
        mass[seed] = 0
        rewards.append(0)
        ratios.append(ratio)
        ...
        while 1:
            ...
            for n in range(len(power)):
                if (dphase[n] > threshold or dphase[n] < -threshold) and activity[n] and mask_gen[n] and hp[n] > 0:
                    failed[n] = True
                    n_fail += 1
                    df += 1
                    if hp[n] == 1:
            ...
            if df:
                feature = torch.column_stack((feature[:,7:], torch.tensor([[hp[n], gamma[n], mass[n], power[n], phase[n], dphase[n], int(failed[n])] for n in range(node)]).to(self.device))).float().to(self.device)
                amplitude, ratio, entropy = self.act(feature, edge_list, edge_weight, act_gen)
                rebal_hp(power, amplitude.clone().detach().cpu().numpy(), act_gen, failed, hp, mass)
                mass[failed] = 0
                rewards[-1] = -df/node
    ...
    def gen_traj_test(self, seed, edge_list, edge_weight, adjm, 
                 init_mask_gen, steady_phase, steady_dphase, init_power, init_activity, init_failed,
                 gamma, h, K, init_mass, threshold, node, max_step, ep_len, init_hp):
