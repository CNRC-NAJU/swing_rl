...
class Actor(torch.nn.Module):
    ...
    def forward(self, features, adj, edge_weight):
        ...
        x = torch.tanh(self.batch_norm2(self.conv2(x, edge_index, edge_weight)))
...
class PPOAgent():
    def __init__(self, num_features=2, num_hiddens=16, num_filters=1, num_nodes=20, device='cuda', lr=5e-4):
    ...
    def act(self, features, adj, edge_weight, activity):
        amplitude, actions, dist, value, entropy = self._get_action(features, adj, edge_weight, activity)
        dist_old = self._get_action_old(features, adj, edge_weight, activity)
    ...
    def _get_action(self, features, adj, edge_weight, activity):
        output, value = self.actor(features, adj, edge_weight), self.critic(features, adj).mean()
    ...
    def _get_action_old(self, features, adj, edge_weight, activity):
        output = self.actor_old(features, adj, edge_weight)
        probs = torch.sigmoid(output)
        dist = Bernoulli(probs[activity])
