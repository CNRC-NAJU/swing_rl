...
class PPOAgent():
    ...
    def gen_traj_train(self, seed, edge_list, edge_weight, init_adjm, 
        ...
        cutting_edge(adjm, np.arange(node)==seed)
        mass[seed] = 0.01
        rewards.append(0)
        ratios.append(ratio)
    ...
    def gen_traj_test(self, seed, edge_list, edge_weight, init_adjm,  
                 mask_gen, steady_phase, steady_dphase, init_power, init_activity, init_failed, 
                 gamma, h, K, init_mass, threshold, node, max_step, ep_len):
        with torch.no_grad():
            self.actor.eval()
