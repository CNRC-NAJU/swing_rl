...
class PPOAgent():
    ...
    def gen_traj_train(self, seed, edge_list, edge_weight, adjm, 
        ...
        activity, inactivity, failed, not_failed = np.copy(init_activity), np.copy(init_failed), np.copy(init_failed), np.copy(init_activity)

        # exert perturbation
        failed[seed] = True
        not_failed[seed] = False
        p_init_fail = 5
        p_fail = 5
        if abs(power[seed]) <= p_init_fail:
            activity[seed], inactivity[seed] = False, True
        ...
        act_gen = activity*mask_gen*not_failed
        amplitude, ratio, entropy = self.act(feature, edge_list, edge_weight, act_gen)
        rebal_partial(power, mass, amplitude.clone().detach().cpu().numpy(), act_gen, failed, p_init_fail)
        rewards.append(0)
        ratios.append(ratio)
        ...
        while 1:
            ...
            not_failed = np.copy(init_activity)
            df = 0
            ...
            for n in range(len(power)):
                if (dphase[n] > threshold or dphase[n] < -threshold) and activity[n]:
                    ...
                    if abs(power[n]) <= p_fail:
                        activity[n], inactivity[n] = False, True
                    n_fail += 1
                    df += 1
            ...
            if df:
                ...
                amplitude, ratio, entropy = self.act(feature, edge_list, edge_weight, act_gen)
                rebal_partial(power, mass, amplitude.clone().detach().cpu().numpy(), act_gen, failed, p_fail)
                rewards[-1] = -df/node
                rewards.append(0)
                ratios.append(ratio)
                entropies.append(entropy)
