...
class Actor(torch.nn.Module):
    ...
    def forward(self, features, adj):
        ...
        # x = self.norm2(x)
        x = torch.tanh(self.final(x)).view(-1)
...
class PPOAgent():
    ...
    def act(self, features, adj, activity):
        ...
        log_probs = dist.log_prob(amplitude)
        log_probs_old = dist_old.log_prob(amplitude).detach()
    ...
    def _get_action(self, features, adj, activity):
        ...
        # dist = Beta(output[activity,0], output[activity,1])
        dist = Normal(output[activity], 0.01)
        # dist = Cauchy(output[activity,0], output[activity,1])
        probs = dist.sample()
        actions = torch.ones(probs.size())
    
        return probs, actions, dist, value, dist.entropy().mean()
