...
class Actor(torch.nn.Module):
    def __init__(self, num_features, num_hiddens, num_filters):
        ...
        self.conv1 = GATv2Conv(num_features, num_hiddens, num_filters, concat=False)
        self.conv2 = GATv2Conv(num_hiddens, num_hiddens, num_filters, concat=False)
...
class PPOAgent():
    ...
    def _get_action(self, features, adj, activity):
        ...
        probs = torch.abs(torch.tanh(output))
        dist = Bernoulli(probs[activity])
        actions = dist.sample()
    
        # return F.tanhshrink(output.clone().detach()), actions, dist, value, dist.entropy().mean()
